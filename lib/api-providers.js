...
const ApiProviders = {
  ...
  // 构建 messages content：文本 + 附件。
  // 图片 → image_url 部件；其它文件（PDF/文本等）→ file 部件（base64 data URL）。
  // 不预设支持性判断：不支持的模型由接口返回错误并显示。
  buildContent(question, attachments) {
    ...
    for (const a of attachments) {
      ...
      if (a.type && a.type.indexOf('image/') === 0) {
        ...
        parts.push({ type: 'file', file: { filename: a.name || 'attachment', file_data: dataUrl } });
      }
    }
    return parts;
  },
  ...
  async streamChat({ provider, model, messages, signal, onDelta }) {    const url = this.chatUrl(provider);
  ...
  // 非流式工具调用（Agent 模式用）：返回 { content, tool_calls }
  async chatTools({ provider, model, messages, tools, signal }) {
    ...
    const m = j.choices && j.choices[0] ? j.choices[0].message : null;
    return {
      content: m ? (m.content || '') : '',
      tool_calls: m && Array.isArray(m.tool_calls) ? m.tool_calls : []
    };
  }
};
